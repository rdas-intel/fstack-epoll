...
lcore_main(void *arg __rte_unused)
{
	...
	printf("%s\n", msgbuf);
	printf("lcore %u using queue %u of each port and total_ports=%u\n", id, (unsigned)q_id, end_port);
	...
	for (;;) {
		...
		for (p = start_port; p < end_port; p++) {
			...
			//printf(" SRC port is %u and DEST port is %u for processing core %u\n", src, dst, id);
...
main(int argc, char **argv)
{
	...
	argc -= ret;
	argv += ret;
	printf("num of ports after  EAL %d\n", num_ports);
